...
class CD():    #change name to html and download what is need then delete txt
    ...
    def OFGL(self):  #open fold and get list
        os.chdir(self.fold)
        Nlist = os.listdir(self.fold)  #name list
        for i in Nlist:
            print(i)
            os.rename(i,i[:-4]+'.html')

    def GINFO(self): #get information
        os.chdir(self.fold)
        Nlist = os.listdir(self.fold)
        for i in Nlist:
            Fname = str(self.fold) +'\\'+str(i)
            self.DINFO(Fname)
#            os.remove(Fname)
...
a=CD(r'F:\项目式文件管理2\1607python学习\161220bjfy\data')
#a.OFGL()
#time.sleep(1)
#print('done changing,sleeping')
a.GINFO()
